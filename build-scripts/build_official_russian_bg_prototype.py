...
def add_glyph(data: dict, glyphs_dir: Path, new_id: int, base: dict, image: Image.Image) -> None:
...
def main() -> int:
    ...
    output_root = project_root / "russian-bg-work"
    ...
    add_glyph(data, glyphs_dir, 173, glyph_map[45], hyphen)
    add_glyph(data, glyphs_dir, 8216, glyph_map[39], apostrophe)
    add_glyph(data, glyphs_dir, 8217, glyph_map[39], apostrophe)
    add_glyph(data, glyphs_dir, 8218, glyph_map[44], comma)
    add_glyph(data, glyphs_dir, 8220, glyph_map[34], quote)
    add_glyph(data, glyphs_dir, 8221, glyph_map[34], quote)
    add_glyph(data, glyphs_dir, 8222, glyph_map[44], compose_double_comma(comma),)
    add_glyph(data, glyphs_dir, 9670, glyph_map[45], make_diamond(7))

    for char, pattern in SYMBOL_PATTERNS.items():
        add_glyph(data, glyphs_dir, ord(char), glyph_map[45], make_bitmap(pattern))

    # Keep regular space but make sure the file is explicit in this branch.
    add_glyph(data, glyphs_dir, 32, glyph_map[32], space)
    ...
    family_name = "Russian Cyrillic BG Prototype V2"
    fb.setupNameTable(
        {
            "familyName": family_name,
            "styleName": "Regular",
            "fullName": f"{family_name} Regular",
            "psName": "RussianCyrillicBGPrototypeV2-Regular",
            "version": "Version 2.0",
            "uniqueFontIdentifier": f"{family_name} Regular 2.0",
    ...
    ttf_path = output_root / "RussianCyrillicBGPrototypeV2.ttf"
    fb.save(ttf_path)
